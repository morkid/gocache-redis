...
// NOTE for v3: gopkg.in/redis.v3 ClusterOptions has no ClusterSlots override
// hook. Two additional constraints come from the v3 source itself:
//  1) Set/Get/IsValid assume Client is set — they dereference r.Client
//     unconditionally and panic when Client is nil. Tests therefore always
//     configure both Client and ClusterClient in cluster tests.
//  2) ClearPrefix calls r.Client.DbSize() before the cluster check.
//
// Result: cluster success paths (returning a nil Err from a working
// ClusterClient) are not exercised for v3. The Client-only path is fully
// covered.
...
// deadClusterClient points at an unreachable address with a short
// DialTimeout. The lazy ClusterClient never connects to miniredis so
// no panic can arise from reentry.
...
// TestCache_ClusterErrorsAfterClose exercises the cluster branch in
// Clear/ClearPrefix/ClearAll with both Client (used by Set/Get/IsValid,
// broken by mr.Close) and a DeadClusterClient (unreachable, errors
// instantly).
